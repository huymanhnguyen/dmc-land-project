...
Public Class frmLichSuKiemKe
    ...
    'Khai báo biến lưu thông tin Lịch sử KIỂM KÊ sử dụng 
    Private dtLichSuKiemKe As New DataTable
    ...
    Public Sub FormatGridContruction()
        ...
        Catch ex As Exception
            strError = ex.Message
            MsgBox(" Hiển thị thông tin lịch sử KIỂM KÊ sử dụng thửa đất" + vbNewLine + strError, MsgBoxStyle.Information, "DMCLand")
        End Try
    End Sub

    Public Sub ShowData()
        'Cập nhật thông tin Lịch sử KIỂM KÊ SỬ DỤNG ĐẤT 
        Dim LichSuKiemKe As New clsThongTinHoSoCapGCNThuaDatLichSu
        LichSuKiemKe.Connection = strConnection
        LichSuKiemKe.MaThuaDatLichSu = strMaThuaDatLichSu
        LichSuKiemKe.MaDonViHanhChinh = strMaDonViHanhChinh
        'Làm sạch dữ liệu trên đối tượng Datatable
        dtLichSuKiemKe.Clear()
        'Nạp thông tin Lịch sử KIỂM KÊ sử dụng vào đối tượng Datatable
        LichSuKiemKe.SelectThongTinLichSuKiemKe(dtLichSuKiemKe)
        'Hiển thị thông tin Lịch sử KIỂM KÊ lên Form
        With Me.grdvw
            .ClearSelection()
            'Trình bày dữ liệu grdvw
            .DataSource = dtLichSuKiemKe
            If dtLichSuKiemKe.Rows.Count > 0 Then
    ...
    Private Sub btnGhi_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnGhi.Click
        If (MessageBox.Show("Bạn có muốn sao chép LỊCH SỬ KIỂM KÊ vào hồ sơ cấp GCN này không?", "DMCLand", MessageBoxButtons.YesNo, MessageBoxIcon.Information) = DialogResult.Yes) Then
            Dim LichSuKiemKe As New clsThongTinHoSoCapGCNThuaDatLichSu
            With LichSuKiemKe
                ...
                'Sao chép thông tin lịch sử KIỂM KÊ thửa đất 
                'vào Hồ sơ cấp GCN hiện thời
                .SaoChepThongTinLichSuKiemKe()
            End With
        End If
        'Thoát Form
        Me.Hide()
