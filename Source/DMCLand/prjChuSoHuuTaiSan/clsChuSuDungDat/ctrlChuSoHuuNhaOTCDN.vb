...
Public Class ctrlChuSoHuuNhaOTCDN
    ...
    Public Sub AddColumnsToChucDoanhNghiep()
        ...
        Catch ex As Exception
            strError = ex.Message
            MsgBox(" Chủ sở hữu Nhà ở (Căn hộ) " & vbNewLine & " Lỗi: " & vbNewLine & strError, MsgBoxStyle.Information, "DMCLand")
        End Try
    End Sub
    ...
    Private Sub FormatGridContruction()
        ...
        Catch ex As Exception
            strError = ex.Message
            MsgBox(" CHỦ SỞ HỮU NHÀ Ở - TỔ CHỨC, DOANH NGHIỆP " & vbNewLine & " Lỗi: " & vbNewLine & strError, MsgBoxStyle.Information, "DMCLand")
        End Try
    End Sub
    ...
    Private Sub btnCapNhat_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnCapNhat.Click
        ...
        If strMaTaiSan = "" Then
            System.Windows.Forms.MessageBox.Show("Bạn phải LỰA CHỌN NHÀ Ở (CĂN HỘ)", "DMCLand", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            Exit Sub
        End If
        With Me
    ...
    End Sub
    ''' <summary>
    ''' Hiển thị thông tin Chủ sở hữu Nhà ở (Căn hộ) lên Grid
    ''' </summary>
    ''' <remarks></remarks>
    Public Sub ShowData()
        ...
        Try
            ...
            With Me
                ...
                If ChuSoHuu.SelectChuSoHuuNhaOByTCDN(dtChuTCDN) = "" Then
    ...
    Private Sub btnGhi_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnGhi.Click
        ...
        If txtMaDoiTuong.Text.Trim = "" Then
            MsgBox("Bạn phải nhập Mã đối tượng sở hữu Nhà ở (Căn hộ)!", MsgBoxStyle.Information, "DMCLand")
            txtMaDoiTuong.Focus()
            Exit Sub
        End If
        With Me
    ...
    End Sub
    ''' <summary>
    ''' Xóa Chủ sở hữu Nhà ở (Căn hộ) (bảng trung gian tblChuSoHuu)
    ...
    Public Sub UpdateData()
        ...
        Try
            ...
            If (strMaTaiSan = "") Then
                strError = "Không tìm thấy Mã Nhà ở (Căn hộ)"
                MessageBox.Show("Lỗi: " & strError, "DMCLand", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                Exit Sub
            End If
            'Chắc chắn rằng tồn tại Mã chủ sở hữu
            If (strMaChu = "") Then
                strError = "Không tìm thấy Mã Chủ sở hữu Nhà ở (Căn hộ)"
                MessageBox.Show("Lỗi: " & strError, "DMCLand", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                Exit Sub
            End If
            'Khai báo nhận chuỗi kết nối Database
            ChuSoHuu.Connection = strConnection
            'GÁN GIÁ TRỊ CHO THUỘC TÍNH CỦA ĐỐI TƯỢNG CHỦ SỞ HỮU NHÀ Ở (CĂN HỘ)
            'Mã Nhà ở (Căn hộ)
            ChuSoHuu.MaTaiSan = strMaTaiSan
            'Mã Chủ Nhà ở (Căn hộ)
            ...
            'Trường hợp thêm mới Chủ sở hữu Nhà ở (Căn hộ)
            If (Me.shortAction = 1) Then
                strUpdateResults = ChuSoHuu.InsertChuSoHuuNhaO(str)
                'Trường hợp Xóa Chủ Sở hữu Nhà ở (Căn hộ)
            ElseIf (Me.shortAction = 3) Then
                strUpdateResults = ChuSoHuu.DeleteChuSoHuuNhaO(str)
            End If
    ...
    Private Sub grdvwTChucDNghiep_CellMouseClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellMouseEventArgs) Handles grdvwTChucDNghiep.CellMouseClick
        ...
        If e.RowIndex >= 0 Then
            Try
                'Hiển thị thông tin Chủ sở hữu Nhà ở (Căn hộ)
